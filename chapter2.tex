...
\index{インストール}\index{Install}\index{Indigo Igloo}\index{VM}

2.1.2項ではROSの簡単なインストール方法について説明したが、仮想マシンの上で開発希望の方は下記手順使用すると簡単にVMでROSの開発できます。Mac、WindowsやLinuxにVirtualBoxとVagrantをインストール前提で簡単に開発始まる可能です。
